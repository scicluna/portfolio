...
export default function Carousel() {
    ...
    return (
        <div className="w-full h-full relative">
            <button className="flex items-center absolute bottom-1/2 translate-y-1/2 w-8 h-full bg-indigo-900 opacity-10 z-10" onClick={e => {
                slideLeft(e)
                ...
                {projects.map((project, i) => {
                    ...
                    return (
                        <div key={i} className={`absolute inset-0 flex items-center justify-center transform ${translateX} transition-transform duration-500 ease-in-out`}>
                            ...
                            <h1 className="absolute bottom-10 text-indigo-300 bg-indigo-900 rounded-lg
...
function Slide({ image, index, showModal, title }) {
    return (
